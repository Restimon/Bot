...
class KissCog(commands.Cog):
    def __init__(self, bot: commands.Bot):
        self.bot = bot

    @app_commands.command(name="kiss", description="Fais un bisou à quelqu'un")
    ...
    async def kiss(self, interaction: discord.Interaction, target: discord.Member):
        if target.bot:
            await interaction.response.send_message("🤖 Les bots n’ont pas besoin d’amour numérique…", ephemeral=True)
            return
        if interaction.user.id == target.id:
            await interaction.response.send_message("💋 Tu ne peux pas t’embrasser toi-même…", ephemeral=True)
            return
